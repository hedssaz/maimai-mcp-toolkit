...
def music_picture(music_id: Union[int, str]) -> Path:
    ...
    if (_path := coverdir / f'{numeric_id}.png').exists():
        return _path
    if numeric_id > 100000:
        base_id = numeric_id - 100000
        if (_path := coverdir / f'{base_id}.png').exists():
            return _path
    if 1000 < numeric_id < 10000 or 10000 < numeric_id <= 11000:
        for _id in [numeric_id + 10000, numeric_id - 10000]:
