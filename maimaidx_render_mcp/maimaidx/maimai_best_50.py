...
class ScoreBaseImage:
    ...
    def whiledraw(
        ...
        for num, info in enumerate(data):
            ...
            rate_text = str(info.rate or 'd').strip()
            rate_icon = score_Rank_l.get(rate_text.lower(), rate_text)
            if not rate_icon or rate_icon.lower() in {'none', 'null', 'nan'}:
                rate_icon = 'D'
            rate = Image.open(maimaidir / f'UI_TTR_Rank_{rate_icon}.png').resize((63, 28))
            ...
            fc_icon = fcl.get(str(info.fc or '').strip().lower())
            if fc_icon:
                fc = Image.open(maimaidir / f'UI_MSS_MBase_Icon_{fc_icon}.png').resize((34, 34))
                self._im.alpha_composite(fc, (x + 154, y + 77))
            fs_icon = fsl.get(str(info.fs or '').strip().lower())
            if fs_icon:
                fs = Image.open(maimaidir / f'UI_MSS_MBase_Icon_{fs_icon}.png').resize((34, 34))
                self._im.alpha_composite(fs, (x + 185, y + 77))
            
            music = mai.total_list.by_id(str(info.song_id))
            dxscore = 0
            if music and info.level_index < len(music.charts):
                dxscore = sum(music.charts[info.level_index].notes) * 3
                dxnum = dxScore(info.dxScore / dxscore * 100) if dxscore else 0
                if dxnum:
            ...
            dxscore_text = f'{info.dxScore}/{dxscore}' if dxscore else f'{info.dxScore}/-'
            self._tb.draw(x + 219, y + 65, 15, dxscore_text, self.t_color[info.level_index], anchor='mm')
...
async def generate(qqid: Optional[int] = None, username: Optional[str] = None) -> Union[MessageSegment, str]:
    ...
    try:
        if username:
            qqid = None
        userinfo = await maiApi.query_user_b50(
            qqid=qqid,
            username=username,
