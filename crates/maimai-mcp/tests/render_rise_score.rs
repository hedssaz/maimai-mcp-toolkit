...
use maimai_mcp::render_tools::{
    ...
    rise_score::{RiseScoreDispatcher, RiseScoreSurface, TOOL_NAME, rise_score_server},
};
...
#[tokio::test]
async fn main_and_public_duplex_preserve_rise_score_surface_shapes()
-> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let service = Arc::new(RiseScoreService::new(
        catalog,
        scores,
        RiseScoreRenderer::new(
    ...
    let main = duplex_call(
        ...
        Arc::clone(&service),
        json!({"qq":"10001","level":"12","source":"sy","algorithm":"legacy"}),
    )
    .await?;
...
async fn duplex_call(
    ...
    let server = rise_score_server(
        contract,
        RiseScoreDispatcher::new(service, surface, fixed_now),
    )?;
    ...
    rpc(
        ...
        json!({"protocolVersion":"2024-11-05","capabilities":{},
            "clientInfo":{"name":"rise-score-test","version":"1"}}),
    )
    .await?;
...
async fn response_server(count: usize) -> Result<String, Box<dyn Error + Send + Sync>> {
    ...
    let body = json!({
        "nickname":"Rise Score User","rating":15000,
        "records":[{
...
fn assert_image(payload: &Value) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    assert_eq!(payload["width"], 1_000);
    let height = payload["height"].as_u64().ok_or("height missing")?;
    assert!((400..=960).contains(&height));
    assert_eq!((height - 260) % 140, 0);
